...
>| set_Basis      | my_bname,my split, mynumcs[], myidxcs[] | int      | 从基组文件中找到所选原子的高斯基函数的总操作安排，会调用到之后定义的函数 |
>| set_Basisspace | split, numcs[], idxcs[]                 | int      | 根据所给基组的种类首先生成储存空间以存放基函数               |
...
>| 名称       | 接受参数                  | 返回类型 | 含义                           |
>| ---------- | ------------------------- | -------- | ------------------------------ |
>| set_I0     | Hiffile, Logfile          | int      | 选取输入文件与输出文件         |
>| set_job    | job, method, basis        | int      | 设置工作任务                   |
...
>
>MatrixXd和Tensor4D类型的变量是一系列的矩阵，进行scf过程时会计算得到它们。
>
>
>
>方法
